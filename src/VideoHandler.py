...
YOUTUBE_DL = r"E:\Videos and Video Utilities\Downloading\youtube-dl.exe"
YOUTUBE_DL = r"C:\Users\civil\Dropbox\Coding\Projects\Python\Tooyunes\src\resources\youtube-dl.exe"

#This will be an object that handles inheritance, getting and setting in a sane way, etc.
GlobalOptions = {"videoStorageDir": "_VideoStore", "videoFormatString": "%(id)s.%(ext)s", "concurrentDownloads": 10}
...
class VideoProcessor:
  ...
  def __init__(self, storageFolder="_VideoStore", concurrentDownloads=None):
    self.youtubeOptions = self.youtubeOptions.copy() #Copy the class instance
  ...
  def processSong(self, songID, progressFunction):
    """ Downloads the info """
    infoThread = threading.Thread(target=self.getInfo, args=(songID,))
    infoThread.start()
    self.downloadSong(songID, progressFunction)
    infoThread.join()

  def downloadSong(self, song, outputFunction=None):
    """
  ...
  def getInfo(self, url, flat=True):
    ...
    try:
      ...
      output = subprocess.check_output([YOUTUBE_DL, "-J", "--flat-playlist" if flat else ""] + self.flattenDict(self.youtubeOptions) + ["--", url], **self.pipeOptions)
